...
namespace android {
    ...
    int V4l2Utils::dump(const uint8_t* data, int size, int width, int height) {
        ...
        if (fd < 0) {
            CAMHAL_LOGE("open %s fail",dump_name.c_str());
            return -1;
        }
        int ret = write(fd,(void*)data, size);
        if (ret == -1) {
            CAMHAL_LOGE("write file fail:%s",strerror(errno));
            close(fd);
            return -1;
        }
        close(fd);
    ...
    void V4l2Utils::set_notify_3A_is_capturing(int videofd, int isCapturing) {
        ...
        if (ret < 0 ) {
            CAMHAL_LOGE("notify to 3A capture status failed: %s\n",strerror(errno));
        }
    }
    int V4l2Utils::set_ae_fps_range(int videofd, int min_fps, int max_fps)
    {
        ...
        if (-1 == ioctl (videofd, VIDIOC_S_EXT_CTRLS, &ctrls)) {
                CAMHAL_LOGE("camera hal set_ae_fps_range failed\n");
                return -1;
        }
        return 0;
    }
