...
int GlobalResource::getFd(char* device_name, int id, std::vector<int> &fds) {
    ...
    fds.push_back(mDeviceFd[id][0]);
    fds.push_back(mDeviceFd[id][1]);
    fds.push_back(mDeviceFd[id][2]);

    return 0;
}

int GlobalResource::getFdEvent(int id, std::vector<int> &fds) {
    bool IsInit = true;
    char property[PROPERTY_VALUE_MAX];
    int counter = 2;
    ...
    if (!IsInit) {
        char name[64];
        int deviceId = property_get_int32("ro.vendor.platform.video.num", 50);
        sprintf(name, "/dev/video%d", deviceId + id);
        ALOGD("%s:init global resource , name:%s", __FUNCTION__, name);
        for (int i = 0; i < HAL_MAX_FD_NUM; i++)
            mDeviceFd[id][i] = open(name, O_RDWR);
...
void GlobalResource::resetFd() {
    for (int i = 0; i < HAL_MAX_CAM_NUM; i++) {
        for (int j = 0; j < HAL_MAX_FD_NUM; j++) {
            if (mDeviceFd[i][j] != -1) {
                close(mDeviceFd[i][j]);
                mDeviceFd[i][j] = -1;
             }
