...
void MIPIVideoInfo::releasebuf_and_stop_picture()
{
    ...
    void MIPIVideoInfo::set_fds(std::vector<int>& fds) {
        switch (mWorkMode) {
            case ONE_FD:
                mOneFd.V4LDevicefd = fds[0];
                break;
            case TWO_FD:
                mTwoFd.mPreviewFd = fds[1];
                mTwoFd.mSnapShotFd = fds[0];
                break;
            case PIC_SCALER:
                if (fds.size() == 2) {
                    mPicScaler.mPreviewFd = fds[1];
                    mPicScaler.mSnapShotFd = fds[0];
                } else if (fds.size() == 3) {
                    mPicScaler.mRecordFd = fds[2];
                    mPicScaler.mPreviewFd = fds[1];
                    mPicScaler.mSnapShotFd = fds[0];
                }
                break;
