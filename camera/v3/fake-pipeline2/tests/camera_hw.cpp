...
static int set_rotate_value(int camera_fd, int value)
{
    ...
    if ((value != 0) && (value != 90) && (value != 180) && (value != 270)) {
        ALOGE("Set rotate value invalid: %d.", value);
        return -1;
    }
    ...
    if (ret<0) {
        ALOGE("Set rotate value fail: %s,errno=%d. ret=%d", strerror(errno),errno,ret);
    }
    return ret ;
}
...
{
        int ret;
        int fd = open(cam_dev->name,O_RDWR);
		if (fd < 0) {
        ...
        if (!(cam_dev->cap.capabilities & V4L2_CAP_STREAMING)) {
                ALOGE( "video%d does not support streaming i/o\n",
                                cam_dev->idx);
        }
        return ret;
}
...
int setBuffersFormat(struct VideoInfo *cam_dev)
{
        ...
        ALOGD("Width * Height %d x %d expect pixelfmt:%.4s, get:%.4s\n",
...
uintptr_t get_frame_phys(struct VideoInfo *vinfo)
{
        ...
        if (ioctl(vinfo->fd, VIDIOC_DQBUF, &vinfo->preview.buf) < 0) {
                switch (errno) {
                        ...
                        default:
                                ALOGE("VIDIOC_DQBUF failed, errno=%d\n", errno);
                                exit(1);
                }
        ALOGE("VIDIOC_DQBUF failed, errno=%d\n", errno);
        }
