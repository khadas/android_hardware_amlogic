...
int staticPipe::constructStaticPipe() {
    ...
    property_get("ro.vendor.isp.enable", property, "true");
    ...
    // main sensor
    if (property_get("ro.vendor.camera.sensor.main", property, NULL) > 0) {
    ...
    //sub sensor
    if (property_get("ro.vendor.camera.sensor.sub", property, NULL) > 0) {
        if (strstr(property, "imx290")) {
...
int staticPipe::fetchPipeMaxResolution(int idx, uint32_t& width, uint32_t &height) {
    if (supportedPipes.empty()) {
        ...
        ALOGE("idx %d overflow, supportedPipes size %zu", idx, supportedPipes.size());
        return -1;
    }
