...
#define TAG AmlogicKeyMintOperation
...
AmlogicKeyMintOperation::AmlogicKeyMintOperation(shared_ptr<AmlogicKeymaster> implementation,
                                               keymaster_operation_handle_t opHandle)
    : impl_(std::move(implementation)), opHandle_(opHandle) {}

AmlogicKeyMintOperation::~AmlogicKeyMintOperation() {
    if (opHandle_ != 0) {
        abort();
    }
}

ScopedAStatus AmlogicKeyMintOperation::updateAad(
...
ScopedAStatus AmlogicKeyMintOperation::update(const vector<uint8_t>& input,
    ...
    if (serialized_size > AMLOGIC_KEYMASTER_SEND_BUF_SIZE) {
    ...
    const size_t max_chunk_size = AMLOGIC_KEYMASTER_SEND_BUF_SIZE - serialized_size;
    output->clear();
...
ScopedAStatus AmlogicKeyMintOperation::finish(const optional<vector<uint8_t>>& input,      //
...
ScopedAStatus AmlogicKeyMintOperation::abort() {
