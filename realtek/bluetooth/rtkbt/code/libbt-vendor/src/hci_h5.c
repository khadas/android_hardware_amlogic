...
static uint8_t h5_complete_rx_pkt(tHCI_H5_CB *h5)
{
    ...
    uint8_t pkt_type = H5_HDR_PKT_TYPE(h5_hdr);
    ...
    if (pass_up)
    {
        ...
        int8_t status = hci_recv_frame(skb_complete_pkt, pkt_type);
...
**  Static functions
******************************************************************************/
static void data_ready_cb_thread(void *UNUSED(arg))
{
    sk_buff *skb;
    uint8_t pkt_type = 0;
...
static void data_retransfer_thread(void *UNUSED(arg))
{
    uint16_t events;
...
**
*******************************************************************************/
uint8_t hci_h5_send_sync_cmd(uint16_t opcode, uint8_t *UNUSED(p_buf), uint16_t UNUSED(length))
{
...
static void h5_retransfer_timeout_handler(union sigval UNUSED(sigev_value)) {
...
static void h5_sync_retrans_timeout_handler(union sigval UNUSED(sigev_value)) {
...
static void h5_conf_retrans_timeout_handler(union sigval UNUSED(sigev_value)) {
...
static void h5_wait_controller_baudrate_ready_timeout_handler(union sigval UNUSED(sigev_value)) {
...
static void h5_hw_init_ready_timeout_handler(union sigval UNUSED(sigev_value)) {
