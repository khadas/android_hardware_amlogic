...
#define RTKBT_RELEASE_NAME "20171219_BT_ANDROID_8.x"
...
static inline int getAltSettings(patch_info *patch_entry, unsigned short *offset, int UNUSED(max_group_cnt))
{
    int n = 0;
    if(patch_entry)
...
void rtk_update_altsettings(patch_info *patch_entry, unsigned char* config_buf_ptr, size_t *config_len_ptr)
{
    ...
    {
        ALOGI("%08zx: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n", i, \
    ...
    {
        ALOGE("rtk_update_altsettings: config len(%x) is not right(%"PRIu32")", data_len, (uint32_t)(config_len-sizeof(struct rtk_bt_vendor_config)));
        return;
    }
    ...
    *config_len_ptr = i+sizeof(struct rtk_bt_vendor_config);
    ...
    size_t tempval = (*config_len_ptr);
    for(i=0;i<=tempval;i+=0x10)
    {
        ALOGI("%08zx: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n", i, \
            config_buf_ptr[i], config_buf_ptr[i+1], config_buf_ptr[i+2], config_buf_ptr[i+3], config_buf_ptr[i+4], config_buf_ptr[i+5], config_buf_ptr[i+6], config_buf_ptr[i+7], \
            config_buf_ptr[i+8], config_buf_ptr[i+9], config_buf_ptr[i+10], config_buf_ptr[i+11], config_buf_ptr[i+12], config_buf_ptr[i+13], config_buf_ptr[i+14], config_buf_ptr[i+15]);
...
uint32_t rtk_parse_config_file(unsigned char** config_buf, size_t* filelen, uint8_t UNUSED(bt_addr[6]), uint16_t UNUSED(mac_offset))
{
    ...
    {
        ALOGE("config len(0x%x) is not right(0x%"PRIu32")", config_len, (uint32_t)(*filelen-sizeof(struct rtk_bt_vendor_config)));
        return 0;
    }
