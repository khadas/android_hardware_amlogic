...
**
*******************************************************************************/
void userial_vendor_ioctl(userial_vendor_ioctl_op_t op, void *UNUSED(p_data))
{
    switch(op)
    {
#if (BT_WAKE_VIA_USERIAL_IOCTL==TRUE)
...
**
*******************************************************************************/
int userial_set_port(char *UNUSED(p_conf_name), char *p_conf_value, int UNUSED(param))
{
...
static void userial_coex_handler(void *UNUSED(context))
{
    RTK_BUFFER* skb_data;
    RTK_BUFFER* skb_type;
    eventfd_t value;
        ...
        if(skb_type) {
            if(*(skb_type->Data) == RTK_DATA_RECEIVED) {
                ...
                if(skb_data) {
                    ...
                    }while(read_length < (int)(skb_data->Length));
                    RtbFree(skb_data);
                }
            }
            else {
...
static void userial_recv_H4_rawdata(void *UNUSED(context))
{
    serial_data_type_t type = 0;
    ssize_t bytes_read;
    uint16_t opcode;
...
static int userial_handle_cmd_event(unsigned char * recv_buffer, int total_length)
{
    ...
    switch (cmd_packet_recv_state) {
        ...
        case RTKBT_PACKET_TYPE:
            cmd_packet_bytes_need = hci_preamble_sizes[HCI_PACKET_TYPE_TO_INDEX(cmd_current_type)];
...
static void* userial_recv_socket_thread(void *UNUSED(arg))
{
...
static void* userial_recv_uart_thread(void *UNUSED(arg))
{
    struct pollfd pfd;
...
static void* userial_coex_thread(void *UNUSED(arg))
{
    struct epoll_event events[64];
    int j;
    while(vnd_userial.thread_running) {
...
int userial_socket_open()
{
    ...
    vnd_userial.event_fd = eventfd(INT_MAX, EFD_NONBLOCK);
