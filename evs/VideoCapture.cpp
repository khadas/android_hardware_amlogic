...
void VideoCapture::collectFrames() {
    ...
    while (mRunMode == RUN) {
        ...
        /*if (mCapturedTestFrame == 0){
            FILE* fp_ref = fopen("/data/local/traces/camera", "w+");
            if (fp_ref) {
                fwrite(mPixelBuffers[buf.index], 1, 3110400, fp_ref);
                fclose(fp_ref);
            } else {
                LOG(ERROR) << "EVS debug: Could not open file camera!";
            }
