...
void fillRGBAFromNV21(const BufferDesc& tgtBuff, uint8_t* tgt, void* imgData, unsigned imgStride __unused) {
    ...
    unsigned char *pY = (unsigned char*)imgData;
    unsigned char *pUV = (unsigned char*)imgData + width * height;
    auto result = libyuv::NV21ToABGR(pY, width, pUV, width, tgt, width * 4, width, height);
    if (result) {
        LOG(ERROR) << "Failed to convert NV21 to RGBA.";
    }
...
void fillRGBAFromNV21(const BufferDesc& tgtBuff, uint8_t* tgt, void* imgData, unsigned imgStride __unused) {
    ...
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j ++) {
            ...
            y = src[rgb_index];
            u = src[nv_start + nv_index ];
            v = src[nv_start + nv_index + 1];
            ...
            tgt[i * width * 4 + 4 * j + 0] = b;
            tgt[i * width * 4 + 4 * j + 1] = g;
            tgt[i * width * 4 + 4 * j + 2] = r;
            tgt[i * width * 4 + 4 * j + 3] = 0xff;
