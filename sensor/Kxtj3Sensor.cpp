...
Kxtj3Sensor::Kxtj3Sensor()
: SensorBase(KXTJ3_DEVICE_NAME, "gsensor"),
      mEnabled(0),
      mInputReader(32)
{
...
int Kxtj3Sensor::enable(int32_t /* handle */, int en)
{
    int newState  = en ? 1 : 0;
    int err = 0;
...
int Kxtj3Sensor::setDelay(int32_t /* handle */, int64_t ns)
{
    if (ns < 0)
        return -EINVAL;
...
int Kxtj3Sensor::update_delay()
{
    ...
    LOGI("Kxtj3Sensor update delay: %dms\n", delay);
...
int Kxtj3Sensor::isActivated(int /* handle */)
{
    return mEnabled;
}

int Kxtj3Sensor::readEvents(sensors_event_t* data, int count)
{
    if (count < 1)
        return -EINVAL;
    ...
    int numEventReceived = 0;       /* The number of events that have been accepted and are to be returned */
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_ABS) {
            ...
            *data++ = mPendingEvent;
            ...
            LOGE("Kxtj3Sensor: unknown event (type=%d, code=%d)",
                    type, event->code);
        }
        mInputReader.next();
    }
...
void Kxtj3Sensor::processEvent(int code, int value)
{
    switch (code) {
...
void Kxtj3Sensor::readCalibration()
{
    if (dev_fd < 0)
        open_device();
