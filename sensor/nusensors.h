...
#include "Gsensor.h"            // Declare the functional interface provided by the driver for HAL. A more abstract file name should be used.
...
#define KXTJ3_DEVICE_NAME     GSENSOR_DEV_PATH
/** Control device file path for akm sensor (M and O sensor). */
...
/** When representing the same acceleration physical quantity, the ratio of the value used by the upper layer of Android and the value sent by the sensor data device. */
#define ACCELERATION_RATIO_ANDROID_TO_HW        (9.80665f / 16384)   //16bit sensor data, the ratio when the measuring range is +-2G
