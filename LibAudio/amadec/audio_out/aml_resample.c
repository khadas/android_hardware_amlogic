...
af_resample_ctl_t af_resampler_ctx;//{0};
static int pcmfd = -1;
/*
...
{
    char value[1028] = {0};
    af_resample_ctl_t *paf_resample_ctl;
    paf_resample_ctl = &af_resampler_ctx;
    memset(paf_resample_ctl, 0, sizeof(af_resample_ctl_t));
    ...
    if (audec->tsync_mode == TSYNC_MODE_PCRMASTER && audec->pcrtsync_enable) {
        paf_resample_ctl->enable_resample = 1;
        if (property_get("vendor.media.libplayer.enable_resample", value, NULL) > 0) {
            paf_resample_ctl->enable_resample = atoi(value);
        }
        amsysfs_set_sysfs_int("sys/class/amaudio/enable_resample", paf_resample_ctl->enable_resample);
...
static void af_resample_linear_coef_get(af_resample_ctl_t *paf_resample_ctl)
{
    int SampNumIn = paf_resample_ctl->SampNumIn;
    int SampNumOut = paf_resample_ctl->SampNumOut;
    int *pCoefArray = paf_resample_ctl->InterpolateCoefArray;
    short *pindex = paf_resample_ctl->InterpolateIndexArray;
    int i;
...
void af_resample_set_SampsNumRatio(af_resample_ctl_t *paf_resample_ctl)
{
    ...
    paf_resample_ctl->LastResamType = resample_type;
    ...
    if (resample_type == RESAMPLE_TYPE_NONE) {
        paf_resample_ctl->SampNumIn = DEFAULT_NUMSAMPS_PERCH;
        paf_resample_ctl->SampNumOut = DEFAULT_NUMSAMPS_PERCH;
    } else if (resample_type == RESAMPLE_TYPE_DOWN) {
        paf_resample_ctl->SampNumIn = DEFAULT_NUMSAMPS_PERCH + default_DELTA_NUMSAMPS;
        paf_resample_ctl->SampNumOut = DEFAULT_NUMSAMPS_PERCH ;
    } else if (resample_type == RESAMPLE_TYPE_UP) {
        paf_resample_ctl->SampNumIn = DEFAULT_NUMSAMPS_PERCH - default_DELTA_NUMSAMPS;
        paf_resample_ctl->SampNumOut = DEFAULT_NUMSAMPS_PERCH;
    }
    af_resample_linear_coef_get(paf_resample_ctl);
    paf_resample_ctl->ReservedSampsValid = 0;
    paf_resample_ctl->OutSampReserveLen = 0;
    paf_resample_ctl->InitFlag = 1;
...
{
    return  paf_resample_ctl->SampNumIn * Nch - paf_resample_ctl->ReservedSampsValid;

}

void  af_get_unpro_inputsampnum(af_resample_ctl_t *paf_resample_ctl, short *buf, int *num)
{
    if (*num >= paf_resample_ctl->ReservedSampsValid) {
        memcpy(buf, paf_resample_ctl->ReservedBuf, paf_resample_ctl->ReservedSampsValid * sizeof(short));
        *num = paf_resample_ctl->ReservedSampsValid;
        paf_resample_ctl->ReservedSampsValid = 0;

    } else { //*num < paf_resample_ctl->ReservedSampsValid
        memcpy(buf, paf_resample_ctl->ReservedBuf, (*num)*sizeof(short));
        memmove(paf_resample_ctl->ReservedBuf,
                paf_resample_ctl->ReservedBuf + (*num),
                (paf_resample_ctl->ReservedSampsValid - (*num))*sizeof(short)
               );
        paf_resample_ctl->ReservedSampsValid -= (*num);
    }
...
void af_get_pcm_in_resampler(af_resample_ctl_t *paf_resample_ctl, short*buf, int *len)
{
    int NumSamp_out = *len;
    int rest_pcm_nums = 0;
    if ((NumSamp_out >= 0) && (NumSamp_out < paf_resample_ctl->OutSampReserveLen)) {
        rest_pcm_nums = paf_resample_ctl->OutSampReserveLen - NumSamp_out;
        memcpy(buf, paf_resample_ctl->OutSampReserveBuf, NumSamp_out * sizeof(short));
        memmove(paf_resample_ctl->OutSampReserveBuf,
                paf_resample_ctl->OutSampReserveBuf + NumSamp_out,
                rest_pcm_nums * sizeof(short));

    } else if (NumSamp_out >= paf_resample_ctl->OutSampReserveLen) {
        memcpy(buf, paf_resample_ctl->OutSampReserveBuf,
               paf_resample_ctl->OutSampReserveLen * sizeof(short));
        NumSamp_out   = paf_resample_ctl->OutSampReserveLen;
        rest_pcm_nums = 0;
    }
    *len = NumSamp_out;
    paf_resample_ctl->OutSampReserveLen = rest_pcm_nums;
}

void  af_resample_process_linear_inner(af_resample_ctl_t *paf_resample_ctl, short *data_in, int *NumSamp_in, short* data_out, int* NumSamp_out, int NumCh)
{

    int index, ChId;
    int NumSampsPerCh_in = (*NumSamp_in) / NumCh;
    int NumSampsPerCh_Pre = paf_resample_ctl->ReservedSampsValid / NumCh;

    //int   NumSampsPerCh_out;
    short buf16_in[MAX_NUMSAMPS_PERCH];
    short buf16_in_valid;
    short *pPreSamps = paf_resample_ctl->ReservedBuf;
    short *pindex = paf_resample_ctl->InterpolateIndexArray;
    int   *pcoef = paf_resample_ctl->InterpolateCoefArray;
    int   input_offset = 0, output_offset = 0;
    int   cur_out_samp_reserve_num = 0;

    if (!paf_resample_ctl->InitFlag) {
        af_resample_set_SampsNumRatio(paf_resample_ctl);
    }
    //indicate at the end of pcm stream
    if (NumSampsPerCh_Pre + NumSampsPerCh_in < paf_resample_ctl->SampNumIn) {
        memcpy(pPreSamps + paf_resample_ctl->ReservedSampsValid, data_in, (*NumSamp_in)*sizeof(short));
        paf_resample_ctl->ReservedSampsValid += (*NumSamp_in);
        //memcpy(data_out,paf_resample_ctl->ReservedBuf,paf_resample_ctl->ReservedSampsValid);
        //*NumSamp_out=paf_resample_ctl->ReservedSampsValid;
        *NumSamp_out = 0;
    } else {
        int NumSampsPerCh_Rest = NumSampsPerCh_Pre + NumSampsPerCh_in - paf_resample_ctl->SampNumIn;
        input_offset += (paf_resample_ctl->SampNumIn - NumSampsPerCh_Pre) * NumCh;
        output_offset += paf_resample_ctl->OutSampReserveLen;
        memcpy(pPreSamps + paf_resample_ctl->ReservedSampsValid, data_in, sizeof(short)*input_offset);
        memcpy(data_out, paf_resample_ctl->OutSampReserveBuf, sizeof(short)*paf_resample_ctl->OutSampReserveLen);
        buf16_in_valid = paf_resample_ctl->SampNumIn;
        for (ChId = 0; ChId < NumCh; ChId++) {
            for (index = 0; index < buf16_in_valid; index++) {
                buf16_in[index] = pPreSamps[NumCh * index + ChId];
            }
            for (index = 0; index < paf_resample_ctl->SampNumOut - 1; index++) {
            ...
            data_out[output_offset + NumCh * (paf_resample_ctl->SampNumOut - 1) + ChId] = buf16_in[buf16_in_valid - 1];
        }
        output_offset += paf_resample_ctl->SampNumOut * NumCh;
        paf_resample_ctl->ReservedSampsValid = 0;

        while (NumSampsPerCh_Rest > paf_resample_ctl->SampNumIn) {
            buf16_in_valid = paf_resample_ctl->SampNumIn;
            for (ChId = 0; ChId < NumCh; ChId++) {
                for (index = 0; index < buf16_in_valid; index++) {
            ...
            NumSampsPerCh_Rest -= paf_resample_ctl->SampNumIn;
            input_offset += paf_resample_ctl->SampNumIn * NumCh;
            output_offset += paf_resample_ctl->SampNumOut * NumCh;
        }
        cur_out_samp_reserve_num = output_offset % (DEFAULT_NUMSAMPS_PERCH * NumCh) ;
        paf_resample_ctl->OutSampReserveLen = cur_out_samp_reserve_num;

        memcpy(paf_resample_ctl->OutSampReserveBuf,
        ...
        *NumSamp_out = output_offset - cur_out_samp_reserve_num;
        ...
        if (input_offset < (*NumSamp_in)) {
            ...
            paf_resample_ctl->ReservedSampsValid = NumSampsPerCh_Rest * NumCh;
        } else {
            paf_resample_ctl->ReservedSampsValid = 0;
        }
    }
...
{
    //memcpy(pPreservedSamps,paf_resample_ctl->OutSampReserveBuf,sizeof(short)*paf_resample_ctl->OutSampReserveLen);
    //memcpy(pPreservedSamps+paf_resample_ctl->OutSampReserveLen,paf_resample_ctl->ReservedBuf,sizeof(short)*paf_resample_ctl->ReservedSampsValid);
    //*SampNum=paf_resample_ctl->OutSampReserveLen + paf_resample_ctl->ReservedSampsValid;
    // paf_resample_ctl->ReservedSampsValid=0;
    //paf_resample_ctl->OutSampReserveLen=0;
    if (paf_resample_ctl->InitFlag != 0) {
        audiodsp_set_pcm_resample_delta(0);
    }
    paf_resample_ctl->InitFlag = 0;
    paf_resample_ctl->LastResamType = 0;
    // adec_print("resample stop INIT_FLAG=%d\n",paf_resample_ctl->InitFlag);
}
...
void af_resample_api_normal(char *buffer, unsigned int *size, int Chnum, aml_audio_dec_t *audec)
{
    ...
    int resample_enable;
    af_resample_ctl_t *paf_resample_ctl;
    short data_in[MAX_NUMSAMPS_PERCH * DEFAULT_NUMCH], *data_out;
    short outbuftmp16[MAX_NUMSAMPS_PERCH * DEFAULT_NUMCH];
    int NumSamp_in, NumSamp_out, NumCh, NumSampRequire = 0;
    ...
    paf_resample_ctl = af_resampler_ctx_get();
    ...
    if (resample_enable) {
        ...
        af_get_pcm_in_resampler(paf_resample_ctl, data_out + outbuf_offset, &NumSampRequire);

        //adec_print("RETURN_SIZE_1:%d    OutSampReserve=%d \n",NumSampRequire,paf_resample_ctl->OutSampReserveLen);

        outbuf_offset += NumSampRequire;
        NumSamp_out   -= NumSampRequire;
        while (NumSamp_out >= DEFAULT_NUMSAMPS_PERCH * NumCh) {
            int delta_input_sampsnum = af_get_delta_inputsampnum(paf_resample_ctl, NumCh);
            NumSamp_in = dsp_pcm_read(audec, (char*)data_in, delta_input_sampsnum * sizeof(short));
            af_resample_process_linear_inner(paf_resample_ctl, data_in, &NumSamp_in, data_out + outbuf_offset, &pcm_cnt, NumCh);

            //adec_print("RETURN_SIZE_2:%d    OutSampReserve=%d \n",pcm_cnt,paf_resample_ctl->OutSampReserveLen);
        ...
        if (NumSamp_out > 0) {
            int delta_input_sampsnum = af_get_delta_inputsampnum(paf_resample_ctl, NumCh);
            NumSamp_in = dsp_pcm_read(audec, (char*)data_in, delta_input_sampsnum * sizeof(short));
            af_resample_process_linear_inner(paf_resample_ctl, data_in, &NumSamp_in, outbuftmp16, &pcm_cnt, NumCh);
            if (pcm_cnt == 0) {
                goto resample_out;
            }
            ...
            memcpy(paf_resample_ctl->OutSampReserveBuf, outbuftmp16 + NumSamp_out, (pcm_cnt - NumSamp_out)*sizeof(short));
            paf_resample_ctl->OutSampReserveLen = (pcm_cnt - NumSamp_out);
        }

    } else {
        if (paf_resample_ctl->OutSampReserveLen > 0) {
            af_get_pcm_in_resampler(paf_resample_ctl, data_out + outbuf_offset, &NumSampRequire);
        ...
        if (paf_resample_ctl->ReservedSampsValid > 0) {
            af_get_unpro_inputsampnum(paf_resample_ctl, data_out + outbuf_offset, &NumSampRequire);
            //adec_print("RETURN_SIZE_5:%d    OutSampReserve=%d \n",NumSampRequire,paf_resample_ctl->ReservedSampsValid);
            outbuf_offset += NumSampRequire;
            NumSamp_out   -= NumSampRequire;
        }

        if ((paf_resample_ctl->OutSampReserveLen == 0) && (paf_resample_ctl->ReservedSampsValid == 0)) {
            af_resample_stop_process(paf_resample_ctl);
        }
