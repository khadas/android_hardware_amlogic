...
static int skt_write(struct audio_stream_out* stream, int fd, const void* p, size_t len) {
    ...
    if ((aml_dev->continuous_audio_mode == 1) && (eDolbyMS12Lib == aml_dev->dolby_lib_type) && (count != 0)) {
        uint64_t input_ns = 0;
        size_t frame_size;
...
static int start_audio_datapath(struct audio_stream_out* stream) {
    ...
    if ((aml_out->flags & AUDIO_OUTPUT_FLAG_PRIMARY) == 0)
        adev->a2dp_active = 1;
    /* check to see if delay reporting is enabled */
    out->enable_delay_reporting = true;//!property_get_bool("persist.bluetooth.disabledelayreports", false);
...
static int stop_audio_datapath(struct audio_stream_out* stream) {
    struct aml_stream_out* aml_out = (struct aml_stream_out*)stream;
    struct a2dp_stream_out* out = aml_out->a2dp_out;
    struct aml_audio_device *adev = aml_out->dev;
    int oldstate = out->state;
...
ssize_t a2dp_out_write(struct audio_stream_out* stream, const void* buffer, size_t bytes) {
    ...
    if (aml_out->hal_rate != out->rate) {
        size_t out_frames = 0;
        out_frames = resample_process(&out->aml_resample, in_frames, (int16_t*) buffer, (int16_t*) buffer);
        if (out_frames == 0) {
            goto finish;
        }
