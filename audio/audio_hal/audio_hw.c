...
static int do_output_standby_l (struct audio_stream *stream)
{
    ...
    if (aml_out->status == STREAM_HW_WRITING) {
        ...
        if (adev->is_TV && aml_out->usecase == STREAM_PCM_DIRECT
                && adev->audio_patching) {
                  ...
                  , audio_format_t output_format)
{
    ...
    ALOGV ("%s out format %#x total_frame %llu latency_frames %d last_frames_postion %llu\n",
           __FUNCTION__, output_format, (unsigned long long) total_frame, latency_frames,
           (unsigned long long) (aml_out->last_frames_postion) );
    ...
    if (config->type == AUDIO_PORT_TYPE_DEVICE) {
        if (config->role == AUDIO_PORT_ROLE_SINK) {
...
#endif
            {
                ...
                ALOGI(" - set sink device[%#x](outport[%s]): volume_dB[%f], gain[%f]",
                        config->ext.device.type, output_ports[outport],
                        volume_in_dB, aml_dev->sink_gain[outport]);
            }
            ...
            if (patch->sinks[0].type == AUDIO_PORT_TYPE_DEVICE) {
                ...
                ALOGI(" - set src device[%#x](inport[%s]): gain[%f]",
                            config->ext.device.type, input_ports[inport],
                            aml_dev->src_gain[inport]);
                ALOGI(" - set sink device[%#x](outport[%s]): volume_dB[%f], gain[%f]",
                            patch->sinks->ext.device.type, output_ports[outport],
                           (float)(config->gain.values[0] / 100), aml_dev->sink_gain[outport]);
