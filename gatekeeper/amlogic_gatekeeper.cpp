...
#define LOG_TAG "AmlogicGateKeeper"
...
#include "amlogic_gatekeeper_ipc.h"
...
AmlogicGateKeeperDevice::AmlogicGateKeeperDevice() {
    GK_context.fd = 0;
    GK_session.ctx = NULL;
    GK_session.session_id = 0;

    int rc = aml_gatekeeper_connect(&GK_context, &GK_session);
    if (rc < 0) {
        LOG(ERROR) << "Error initializing amlogic session: " << rc;
    }
...
AmlogicGateKeeperDevice::~AmlogicGateKeeperDevice() {
    aml_gatekeeper_disconnect(&GK_context, &GK_session);
}
...
::ndk::ScopedAStatus AmlogicGateKeeperDevice::enroll(
...
::ndk::ScopedAStatus AmlogicGateKeeperDevice::verify(
...
::ndk::ScopedAStatus AmlogicGateKeeperDevice::deleteUser(int32_t uid) {
    if (error_ != 0) {
        return ndk::ScopedAStatus(AStatus_fromServiceSpecificError(ERROR_GENERAL_FAILURE));
    }
...
::ndk::ScopedAStatus AmlogicGateKeeperDevice::deleteAllUsers() {
    if (error_ != 0) {
        return ndk::ScopedAStatus(AStatus_fromServiceSpecificError(ERROR_GENERAL_FAILURE));
    }
...
gatekeeper_error_t AmlogicGateKeeperDevice::Send(uint32_t command, const GateKeeperMessage& request,
    ...
    int rc = aml_gatekeeper_call(&GK_session, command, send_buf, request_size, recv_buf, &response_size);
    if (rc < 0) {
