...
class TexturePainter : public BasePainter {
public:
    TexturePainter();
...
protected:
    ...
    virtual const char* TextureName();
...
private:
    ...
    GLint mTexCoordLen;
    GLint mTexCoordNum;
    GLenum mTexCoordType;
    GLboolean mTexCoordNormalized;
    GLsizeiptr mTexCoordSize;

    //---Texture
    size_t mTexturesNum;
    Texture *mTextures;

    EGLImageKHR *mTextureEGLImages;
    GLuint *mTextureIDs;

    int mTextureObsoleted;
    //---Texture
    ...
    int InitTextureData(size_t num);
    int DeinitTextureData(size_t num);

    GLenum InitTexture();
    GLenum DeinitTexture();

    GLenum UpdateMVP(size_t index);
    GLenum DrawOneTexture();

    DISALLOW_COPY_AND_ASSIGN(TexturePainter);
};
