...
class ImageReader {
public:
    ImageReader(MemType memType, const char **paths, size_t num);
    ~ImageReader();
...
private:
    ...
    int InitDatas(MemType memType);
    int DeinitDatas();

    int ReadDateFromFile(void *data, const char *filePath,
            long offset, size_t size);
    int ParseBmpInfo(const char *bmpPath, size_t &width,
            size_t &height, size_t &pixelLen, size_t &size);

    int ParseTexture(const char *path, Texture &texture);

    int MallocTexture(Texture &texture);
    int FreeTexture(Texture &texture);

    int LoadTexture(const char *path, Texture &texture);
    int InitTexture(const char *path, Texture &texture);

    int InitTextures(const char **paths, size_t num);
    int DeinitTextures();
